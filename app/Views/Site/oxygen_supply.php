<?php
    ...
    $this->section('head');
        echo "<title>Oxygen Supply - Go Corona Go</title>";
...
<div class="h1 text-center my-3"><i class="fas fa-lungs"></i>Oxygen Supply</div>
...
<div class="col-md-6 my-4 m-auto">
    <div  class="row g-3 p-2">
        ...
        <div class="col-md-3">
            <button class="btn btn-success" id="oxygen_supply_search" onclick="fetch_oxygen_suppliers()">Search</button>
        </div>
    </div>
    <hr>
</div>
...
<!-- results container -->
<div class="container-lg" id="result_area_oxygen"></div>
...
<?= $this->section('custom_script') ?>
    ...
    <script>
        ...
        $( document ).ajaxStart(function() {
            ...
            $( "#result_area_oxygen" ).html("");

            $( "#oxygen_supply_search").prop("disabled", true).html("Searching...")
        });

        

        function fetch_oxygen_suppliers() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $("#result_area_oxygen").html("")
                    $( "#oxygen_supply_search").prop("disabled", false).html("Search")


                    if (response_count == 0) {
                        $("#result_area_oxygen").css("display", "block")
                                                .addClass("text-muted")
                                                .addClass("text-center")                                               
                                                .html("<i class='fas fa-heart-broken fa-4x mb-2 mt-5'></i>")
                        
                        resultLine = document.createElement('div')
                        $(resultLine).html("<h4>No information found, try searching nearby areas </h4>").appendTo($("#result_area_oxygen"))
                    }

                    response.forEach(element => {
                        // style="display:flex; flex-wrap: wrap;"
                        $("#result_area_oxygen").css("display", "flex")
                                                .css("flex-wrap", "wrap")
                                    

                        wrapperDiv = document.createElement('div')  
                        $(wrapperDiv).addClass('col-md-6')
                                    .appendTo($("#result_area_oxygen"))
                        ...
                        let tableColumn = ["<i class='fas fa-address-card'></i> Address", "<i class='fas fa-asterisk'></i> Description", "<i class='fas fa-lungs'></i> Oxygen Amount", "<i class='fas fa-check'></i> Verification", "<i class='fas fa-history'></i> Updated", "<i class='fas fa-globe'></i> Place"]
                        let tableContents = ['address', 'description', 'oxygen_left', 'verification', 'ago', 'pincode']
        ...
        window.onload =  fetch_oxygen_suppliers()
